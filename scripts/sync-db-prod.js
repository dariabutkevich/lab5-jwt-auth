...
async function initial() {
    try {
        // Нам не нужно showAllTables, так как db.sequelize.sync({ alter: true })
        // гарантирует, что таблица roles будет создана, если она не существует.
        ...
        // В случае ошибки (например, таблица еще не создана) выведем предупреждение.
        console.error('Warning: Could not check or create roles. Error:', err.message);
    }
}
...
async function runSync() {
    try {
        ...
        // { alter: true } - безопасный способ для Production.
        await db.sequelize.sync({ alter: true }); 
        
        console.log("DB Sync complete. Initializing roles...");

        // Инициализация ролей
        await initial(); 

        console.log("Post-deploy setup successful. Tables are ready.");
        process.exit(0); 
        
    } catch (err) {
        console.error("FATAL: Production DB sync failed:", err.stack); // Выводим stack для полной информации
        process.exit(1); 
    }
}
